...
def duplicate(o, pizza_name):
    ...
    for i in range(0, len(o)):
        print(o[i][1])
        print(pizza_name)
        if pizza_name is o[i][1]:
            choice = validate_string("You have already ordered this pizza. Would you like to change the quantity [yes/no]? ", 1, 1).upper()
            if choice is ["YES","Y"]:
                updated = validate_integer("How many {} pizza would you like to order? --> ".format(o[i][0]), 1, 10)
                o[i][0] = updated
                print("You now have {} {} pizzas".format(updated, o))
                return True
            elif choice is ["NO", "N"]:
                return False
        else:
...
def customers_order(m, o):
    ...
    while run is True:
        ...
        if result is not None:
            ...
            print("duplicate_function", duplicate_function)
            ...
            while incorrect_choice is True:
                if duplicate_function is True:
                    ...
                    print("I don't know what duplicate function is")
                    return None
        else:
...
def customer_details(c):
    ...
    :param c: list of customer's detail, sub-list is [str, str or str, str, str, str, str, str]
    :return: 3 or 0
    """
    ...
    while run is True:
        receive_order = validate_string("How are you going to receive your "
                                        "order?\n"
                                        "- Delivery or Pick Up --> ", 6, 7).upper()
        print("=" * 70)
        # Collecting details for delivery
        if receive_order == ["DELIVERY","D"]:
...
def complete_order(o, e):
    ...
    :param e: return from c, sub-list is [None or int]
    :return: None
